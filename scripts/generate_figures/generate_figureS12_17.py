...
def get_figure_number(feature, filtered):
    d = dict()
    d['facingness'] = {'filtered': 'S14', 'unfiltered': 'S15'}
    d['transitivity'] = {'filtered': 'S12', 'unfiltered': 'S13'}
    d['communication'] = {'filtered': 'S16', 'unfiltered': 'S17'}
    return d[feature][filtered]
...
for feature, threshold in itertools.product(['transitivity', 'facingness', 'communication'], ['filtered', 'unfiltered']):
    figure_number = get_figure_number(feature, threshold)
    # Open the canvas
    ...
    for i, (subj, figure) in enumerate(zip(range(4), ['a', 'b', 'c', 'd'])):
        ...
        for view in views:
            file = f"{surface_path}/sub-{sid}/{threshold}/sub-{sid}_dropped-featurewithnuisance-{feature}_view-{view}_hemi-lh.png"
            add_img(c, file,
